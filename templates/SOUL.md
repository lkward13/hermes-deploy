...
- Clover POS: CLOVER_ACCESS_TOKEN + CLOVER_MERCHANT_ID (sandbox vs prod via CLOVER_SANDBOX). Use `skills/clover/clover_lookup.py` for orders, customers, items (inventory), payments, employees, and merchant info. Read-only — no writes. Example: `python3 ~/.hermes/skills/clover/clover_lookup.py --list-orders --limit 10`. If `CLOVER_ACCESS_TOKEN` is empty the customer hasn't connected Clover; tell them to connect it in the NoDesk portal rather than asking for credentials.
- JobNimbus (contractor CRM): JOBNIMBUS_API_KEY. Use `skills/jobnimbus/jobnimbus_lookup.py` for contacts, jobs, tasks, activities (notes), estimates, and invoices. Read-only — no writes. Examples: `python3 ~/.hermes/skills/jobnimbus/jobnimbus_lookup.py --search "Smith"` and `... --list-jobs --limit 20`. If `JOBNIMBUS_API_KEY` is empty the customer hasn't connected JobNimbus; tell them to connect it in the NoDesk portal rather than asking for credentials.
